...
def _wait(fs, return_early_n, max_direct_query_n,
    ...
    not_done_futures = [f for f in fs if f._state not in [ResponseFuture.State.Success,
                                                          ResponseFuture.State.Error]]
    if len(not_done_futures) == 0:
        return fs, []

    storage_config = npw.config.default()
    ...
    for callset_id in present_callsets:
        ...
        f_to_wait_on = []
        for f in fs:
            if f._state in [ResponseFuture.State.Success, ResponseFuture.State.Error]:
