...
def create_security_group(name="numpywren.group"):
    ...
    if (len(groups) == 0):
        ...
        resp = client.authorize_security_group_ingress(GroupId=group_id, IpProtocol="tcp", CidrIp="0.0.0.0/0",
                                                       FromPort=0, ToPort=9999)
    else:
        group = groups[0]
...
def launch_and_provision_redis(config=None):
    ...
    pywren_config = npw.config.default()
    ...
    user_data = user_data.format(redis_init=redis_init_b64, cloud_agent_conf=cloud_agent_conf_64,
                                 redis_conf=redis_conf_b64, aws_region=aws_region)
    ...
    instances = _create_instances(1, aws_region, spot_price, ami=ami, instance_type=instance_type,
                                  block_device_mappings=None, security_group_ids=[group_id], ebs_optimized=True,
                                  availability_zone=None, instance_profile=instance_profile_dict, user_data=user_data,
                                  key_name=key_name)
    inst = instances[0]
    inst.reload()
    inst.create_tags(
    ...
    def client(self):
        ...
        if (self._cache):
            if (self._conn is None):
                self._conn = redis.StrictRedis(host=host, port=self.port, password=self.password,
                                               socket_timeout=self.socket_timeout, db=self.db)
            return self._conn
        else:
            return redis.StrictRedis(host=host, port=self.port, password=self.password,
                                     socket_timeout=self.socket_timeout, db=self.db)
