...
def put(key, value, s3=True, s3_bucket="bogdan-pywren-830"):
    print(key, value)
    if (s3):
...
def get(key, s3=True, s3_bucket="bogdan-pywren-830"):
    if (s3):
        client = boto3.client('s3')
...
def incr(key, amount=1, s3=True, s3_bucket='bogdan-pywren-830'):
    if (s3):
        # read from S3
...
def decr(key, amount, s3=True, s3_bucket='bogdan-pywren-830'):
    if (s3):
        # read from S3
        response = get(key, s3_bucket=s3_bucket)
        response = int(response)
...
class LambdaPackProgram(object):
    ...
    def __init__(self, program, config, num_priorities=1, eager=False, block_sparse=False):
        ...
        put(self.hash, PS.NOT_STARTED.value)
    ...
    def get_node_status(self, expr_idx, var_values):
        s = get(self._node_key(expr_idx, var_values))
        if (s == None):
            s = 0
        return NS(int(s))
